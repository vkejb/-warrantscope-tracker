...
新的 [短期大漲事件研究 V0.1](surge_event_study_v01/README.md) 不沿用上述價量突破條件，而是從全體合格 stock-day 反推 +15%／10 日事件的事前共通點。2020–2022 鎖定出高 ATR、上升中的 20 日均線、20 日動能與事前量能擴張四項特徵；2023–2024 的事件命中 lift 雖為 2.683 倍，但 Day10 報酬增量的月叢集信賴區間仍跨 0，三萬元成交代理為 -13.30%，因此正式判定失敗並未開啟 2025。這個結果顯示它目前捕捉的是「容易大幅衝高的高波動路徑」，尚不是穩定可變現的交易 edge。

新的 [相對低點反轉事件研究 V0.1](reversal_event_study_v01/README.md) 只用 T 日以前資料定義 V 型急跌止穩與早期 N／雙底回測，主標籤固定為 T+1 Open 後 10 日內 Close 先到 +8%、未先到 -5%。2023–2024 鎖定驗證中，V 成功率 19.92%、毛 Close-rule PF 0.870，三萬元代理 -12.25%；N 成功率 24.12%、毛 PF 1.134，但信賴區間跨零且三萬元代理 -1.66%、壓力情境 -23.64%。兩者皆失敗，因此未開啟 2025。診斷顯示 V 型確認日漲幅有值得後續預註冊的線索，但單純爆量並未穩定延續，不能直接當成新策略。
