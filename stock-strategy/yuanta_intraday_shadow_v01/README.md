...
移動停利；若部位曾為負報酬，之後從最差點回升至少 2 個百分點且扣成本後
轉正，也立即平倉。另保留方向反轉與 13:20 強制平倉；若強制平倉前 60 秒
內沒有可用報價，該筆結果視為無法評分，不使用更早的價格灌入績效。結果
同樣只屬於樣本內探索。

`direction_signal_validation.py` 會把每檔股票每天第一個可負擔的合格訊號各自
獨立回放，以增加方向判斷樣本。每筆都套用相同的 19 萬元上限與出場規則，
但訊號可能同時重疊，因此其損益加總只作診斷，不能視為單一 19 萬元帳戶
可執行的投資組合績效。
